// src/components/Thematic/LandslideRisk.jsx
...
const LandslideRisk = () => {
  ...
  useEffect(() => {
    if (!schema) {
      console.log("⚠️ LandslideRisk: no schema, toggle not registered");
      return;
    }

    console.log("✅ LandslideRisk: registering toggle for schema =", schema);

    const updateButton = (active) => {
      const btn = document.getElementById("btnLandslideRisk");
      if (btn) btn.classList.toggle("active-tool", active);
    };

    window.toggleLandslideRisk = async () => {
      console.log("🖱️ toggleLandslideRisk called");

      if (!map || !schema || isBusy.current) {
        console.log("⏸️ LandslideRisk: map/schema missing or busy");
        return;
      }
      isBusy.current = true;
      ...
      try {
        // === Turn OFF
        if (window.landslideRiskLayerGroup) {
          console.log("🔄 Removing LandslideRisk layer");
          if (map.hasLayer(window.landslideRiskLayerGroup)) {
            map.removeLayer(window.landslideRiskLayerGroup);
          }
          window.landslideRiskLayerGroup = null;
          updateButton(false);
          window.removeThematicLegend?.("landslide-risk");
          isBusy.current = false;
          return;
        }

        // === Turn ON
        const url = `${API}/single-table?schema=${schema}&table=LandslideRisk`;
        ...
        if (!props.hasOwnProperty("rating")) {
          console.warn("⚠️ LandslideRisk: no 'rating' column, showing picker");
        ...
        console.error("❌ LandslideRisk fetch error:", err);
      } finally {
        isBusy.current = false;
      }
    };
  ...
  const renderLayer = (geojson, column) => {
    console.log("🎨 Rendering LandslideRisk by column:", column);
    ...
    // Sort categories: high > moderate > low > others
  ...
  const handleApplyColumn = (col) => {
    console.log("📊 User applied LandslideRisk column:", col);
    setSelectedColumn(col);
    setShowColumnPopup(false);
    if (geojsonData) {
      renderLayer(geojsonData, col);
      document.getElementById("btnLandslideRisk")?.classList.add("active-tool");
    }
  };
